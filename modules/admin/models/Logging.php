...
class Logging extends RedActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('user_id, username, request, type, controller, action, time, date, ip', 'required'),
            array('user_id, time', 'numerical', 'integerOnly' => true),
            array('username', 'length', 'max' => 20),
            array('request', 'length', 'max' => 255),
            array('type, date', 'length', 'max' => 10),
            array('controller, action', 'length', 'max' => 50),
            array('ip', 'length', 'max' => 15),
            array('param', 'safe'),
            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            array('id, user_id, username, request, param, type, controller, action, time, date, ip', 'safe', 'on' => 'search'),
        );
    }
